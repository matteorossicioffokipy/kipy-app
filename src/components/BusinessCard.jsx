...
import { Mail, Pencil, Save, MessageCircle, Eye, QrCode } from 'lucide-react';

export default function BusinessCard({ config, user, supabase, fetchDati }) {
...
END:VCARD`;
  ...
  const shareEmail = () => {
    ...
    const body = encodeURIComponent(
      ...
      (visibili.indirizzo ? `Indirizzo: ${form.indirizzo}\n` : '') +
      (visibili.piva ? `P.IVA: ${form.piva}\n` : '') +
      (visibili.orari ? `Orari: ${form.orari}\n` : '')
    );
  ...
  const shareWhatsApp = () => {
    const text = encodeURIComponent(
      ...
      (visibili.indirizzo ? `📍 ${form.indirizzo}\n` : '') +
      (visibili.piva ? `🧾 P.IVA: ${form.piva}\n` : '') +
      (visibili.orari ? `🕐 ${form.orari}\n` : '')
    );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', fontFamily: "'Baloo 2', sans-serif" }}>
      ...
      <div style={cardStyle}>
        ...
        <div style={cardHeaderStyle}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <div>
              <div style={{ fontSize: '20px', fontWeight: '800', color: '#1E293B', lineHeight: 1.2 }}>
                {form.nome} {form.cognome}
              </div>
              <div style={{ fontSize: '13px', color: '#5D5C9E', fontWeight: '600', marginTop: '2px' }}>
                {form.azienda || 'La tua Azienda'}
              </div>
              {config.settore && (
      ...
      <div style={sectionBox}>
        <div style={sectionTitleStyle}>
          <Eye size={15} color="#5D5C9E" /> Cosa mostrare nel biglietto
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {[
            { key: 'telefono', label: 'Telefono', valore: form.telefono },
            { key: 'email', label: 'Email', valore: form.email_business },
            { key: 'sito', label: 'Sito web', valore: form.sito },
            { key: 'indirizzo', label: 'Indirizzo', valore: form.indirizzo },
            { key: 'piva', label: 'P.IVA', valore: form.piva },
            { key: 'orari', label: 'Orari', valore: form.orari },
          ].map(item => (
            <div key={item.key} style={toggleRowStyle} onClick={() => toggleVisibile(item.key)}>
              <div>
                <div style={{ fontSize: '13px', fontWeight: '700', color: '#1E293B' }}>{item.label}</div>
                {item.valore
                  ? <div style={{ fontSize: '11px', color: '#94A3B8' }}>{item.valore}</div>
                  : <div style={{ fontSize: '11px', color: '#CBD5E1' }}>Non inserito — vai su Modifica</div>
                }
              </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px' }}>
        <button onClick={() => setEditing(!editing)} style={actionBtn('#EEEEF8', '#5D5C9E')}>
          <Pencil size={18} />
          <span style={{ fontSize: '12px', fontWeight: '700' }}>Modifica</span>
        </button>
      ...
      {editing && (
        <div style={sectionBox}>
          <div style={sectionTitleStyle}>
            <Pencil size={15} color="#5D5C9E" /> Modifica dati
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
              <div>
                <label style={labelStyle}>Nome</label>
                <input style={inputStyle} placeholder="Nome" value={form.nome}
                  onChange={e => setForm({ ...form, nome: e.target.value })} />
              </div>
              <div>
                <label style={labelStyle}>Cognome</label>
                <input style={inputStyle} placeholder="Cognome" value={form.cognome}
                  onChange={e => setForm({ ...form, cognome: e.target.value })} />
              </div>
            </div>
            <div>
              <label style={labelStyle}>Telefono</label>
              <input style={inputStyle} placeholder="Es: +39 333 444 5566" value={form.telefono}
                onChange={e => setForm({ ...form, telefono: e.target.value })} />
            </div>
            <div>
              <label style={labelStyle}>Email</label>
              <input style={inputStyle} placeholder="email@esempio.com" value={form.email_business}
                onChange={e => setForm({ ...form, email_business: e.target.value })} />
            </div>
            <div>
              <label style={labelStyle}>Sito web</label>
              <input style={inputStyle} placeholder="www.esempio.com" value={form.sito}
                onChange={e => setForm({ ...form, sito: e.target.value })} />
            </div>
            <div>
              <label style={labelStyle}>Indirizzo</label>
              <input style={inputStyle} placeholder="Via Roma 1, Milano" value={form.indirizzo}
                onChange={e => setForm({ ...form, indirizzo: e.target.value })} />
            </div>
            <div>
              <label style={labelStyle}>P.IVA</label>
              <input style={inputStyle} placeholder="IT00000000000" value={form.piva}
                onChange={e => setForm({ ...form, piva: e.target.value })} />
            </div>
            <div>
              <label style={labelStyle}>Orari</label>
              <input style={inputStyle} placeholder="Lun-Ven 9:00-18:00" value={form.orari}
                onChange={e => setForm({ ...form, orari: e.target.value })} />
            </div>
            <div style={{ display: 'flex', gap: '10px', marginTop: '6px' }}>
              <button onClick={() => setEditing(false)} style={cancelBtn}>Annulla</button>
...
const cardStyle = {
  ...
  boxShadow: '0 4px 24px rgba(93,92,158,0.1)',
  border: '1.5px solid rgba(93,92,158,0.12)',
};
...
const logoStyle = {
  ...
  objectFit: 'contain', background: '#F8F9FF',
  border: '1.5px solid #EEEEF8', padding: '6px', flexShrink: 0,
};

const avatarStyle = {
  width: '60px', height: '60px', borderRadius: '16px',
  background: '#5D5C9E', display: 'flex', alignItems: 'center',
  justifyContent: 'center', fontSize: '22px', fontWeight: '800',
  color: 'white', flexShrink: 0,
};

const qrSmallStyle = {
  background: '#F8F9FF', border: '1.5px solid #EEEEF8',
  borderRadius: '16px', padding: '10px', flexShrink: 0,
};
...
const toggleRowStyle = {
  ...
  padding: '8px 0', cursor: 'pointer', borderBottom: '1px solid #F8FAFC',
};

const toggleStyle = (attivo) => ({
  width: '42px', height: '24px', borderRadius: '12px',
  background: attivo ? '#5D5C9E' : '#E2E8F0',
  position: 'relative', transition: 'background 0.2s', flexShrink: 0,
});

const toggleThumbStyle = (attivo) => ({
  position: 'absolute', top: '3px',
  left: attivo ? '21px' : '3px',
...
const actionBtn = (bg, color) => ({
  ...
  gap: '6px', fontFamily: "'Baloo 2', sans-serif",
...
const inputStyle = {
  ...
  fontFamily: "'Baloo 2', sans-serif", color: '#1E293B',
